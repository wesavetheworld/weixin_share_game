...
<body>
	...
	<div id="">
		<div class="ui raised segment">
			<a class="ui teal  ribbon label">提现完成</a>
			<br/><br/>
			<div class="clear"></div>
  ...
  ?>
			...
			<table id="user_list_table" class="ui teal  table">
				...
				<tbody >
					...
					<tr>
						...
						<td><?=$value['u_cash']?></td>
						<td><?=$value['time']?></td>  
						<td style="color: #19F909;cursor: pointer;" data-uid="<?=$value['u_id']?>" data-tel="<?=$value['u_tel']?>" data-name = "<?=$value['u_name']?>" data-cash="<?=$value['u_cash']?>" data-email="<?=$value['u_email']?>" >
							已完成<i class="fa fa-check"></i>
						</td>
					</tr>
					<?php } ?>
    ...
    if ($thispage<=1) { 
      ...
      echo'<a class="icon item" href="http://share.kejixi.com/index.php/admin/ok_cash/'.($thispage*1-1*1).'?words='.$words.'"><i class="left arrow icon"></i></a>';
    }
...
if ($page_toatl>9) {
    ...
    for ($i = $starpage; $i <= $endpage; $i++) {
      if ($i==$thispage) {
        echo'<a class="active item" href="http://share.kejixi.com/index.php/admin/ok_cash/'.$i.'?words='.$words.'">'.$i.'</a>';
      }else{
        echo'<a class="item" href="http://share.kejixi.com/index.php/admin/ok_cash/'.$i.'?words='.$words.'">'.$i.'</a>';
      }
    };

    if ($thispage<$page_toatl) {
      echo'<a class="icon item" href="http://share.kejixi.com/index.php/admin/ok_cash/'.($thispage*1+1).'?words='.$words.'"><i class="right arrow icon"></i></a>';
    }else{
...
<script type="text/javascript">
	...
	$("#select_data").click(function(){
 		...
 		window.location.href="http://share.kejixi.com/index.php/admin/ok_cash/1?words="+words+"";
	});
    ...
    window.location.href='http://share.kejixi.com/index.php/admin/ok_cash/'+num+'?&words='+words+'';   
}); 
